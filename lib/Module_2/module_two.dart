...
import 'package:code_quest_pre/Module_2/Chapter_1/chapter_one.dart';
import 'package:code_quest_pre/Module_2/Chapter_2/chapter_two.dart';
import 'package:code_quest_pre/Module_2/Chapter_3/chapter_three.dart';
import 'package:code_quest_pre/Module_2/module_two_progress.dart';
import 'package:flame_audio/flame_audio.dart';
import 'package:flutter/material.dart';
import 'dart:ui';

class ModuleTwoMap extends StatefulWidget {
  ...
  @override
  _ModuleTwoMapState createState() => _ModuleTwoMapState();
}

class _ModuleTwoMapState extends State<ModuleTwoMap> {
  final double button1Top = 250;
  final double button1Left = 110;
  final double button2Top = 565;
  final double button2Left = 945;
  ...
  bool _isChapterAvailable(String chapter) {
    if (chapter == 'Chapter I') {
      ...
      return ModuleTwoProgress.isChapterCompleted(1) ||
          ModuleTwoProgress.isChapterPermanentlyUnlocked(
              1); // Chapter II remains unlocked once Chapter I is done
    } else if (chapter == 'Chapter III') {
      return ModuleTwoProgress.isChapterCompleted(2) ||
          ModuleTwoProgress.isChapterPermanentlyUnlocked(
  ...
  Widget _buildRoundButton(String text, {int? requiredRound}) {
    ...
    if (ModuleTwoProgress.isRoundCompleted(chapterNumber, roundNumber)) {
      isActive = true;
      tooltipMessage = 'Round completed';
    } else {
      // Sequential unlocking logic for rounds
      if (requiredRound != null) {
        if (!ModuleTwoProgress.isRoundCompleted(chapterNumber, requiredRound)) {
          isActive = false;
          tooltipMessage =
    ...
    int stars =
        ModuleTwoProgress.getStarsForChapter(chapterNumber, roundNumber);
    String badge = ModuleTwoProgress.getBadgeForStars(stars);
  ...
  Widget build(BuildContext context) {
    ...
    final double button1Top = screenHeight * (isMobile ? 0.25 : 0.4);
    final double button1Left = screenWidth * (isMobile ? 0.11 : 0.001);

    final double button2Top = screenHeight * (isMobile ? 0.57 : 0.78);
    final double button2Left = screenWidth * (isMobile ? 0.96 : 0.47);

    final double button3Top = screenHeight * (isMobile ? 0.18 : 0.28);
    final double button3Left = screenWidth * (isMobile ? 1.05 : 0.5);
    return Scaffold(
      body: Stack(
        children: [
          AnimatedOpacity(
            ...
            child: Stack(
              children: [
                Positioned.fill(
                  child: FittedBox(
                    fit: BoxFit.cover,
                    child: Image.asset('assets/images/ModulesBG/Module2BG.png'),
                  ),
                ),
                Positioned(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        _buildChapterWithTopics(
                                          'Module II: Getting Input from the Keyboard',
                                          [
                                            "Welcome to Module II! In this module, you'll explore various ways to handle user input in Java. Get ready to dive into topics such as Using BufferedReader to Get Input, Reading Data with the Scanner Class, Using JOptionPane for Input, and Parsing Integers from Strings using Java's built-in methods. These skills are essential for creating interactive programs, so let’s get started and master how to capture and process input in Java!"
                                          ],
                                        ),
                                      ],
                                    ),
