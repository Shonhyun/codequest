...
import 'package:code_quest_pre/Module_4/Chapter_1/chapter_one.dart';
import 'package:code_quest_pre/Module_4/Chapter_2/chapter_two.dart';
import 'package:code_quest_pre/Module_4/Chapter_3/chapter_three.dart';
import 'package:code_quest_pre/Module_4/module_four_progress.dart';
import 'package:flame_audio/flame_audio.dart';
import 'package:flutter/material.dart';
import 'dart:ui';

class ModuleFourMap extends StatefulWidget {
  ...
  @override
  _ModuleFourMapState createState() => _ModuleFourMapState();
}

class _ModuleFourMapState extends State<ModuleFourMap> {
  ...
  bool _isChapterAvailable(String chapter) {
    if (chapter == 'Chapter I') {
      ...
      return ModuleFourProgress.isChapterCompleted(1) ||
          ModuleFourProgress.isChapterPermanentlyUnlocked(
              1); // Chapter II remains unlocked once Chapter I is done
    } else if (chapter == 'Chapter III') {
      return ModuleFourProgress.isChapterCompleted(2) ||
          ModuleFourProgress.isChapterPermanentlyUnlocked(
  ...
  Widget _buildRoundButton(String text, {int? requiredRound}) {
    ...
    if (ModuleFourProgress.isRoundCompleted(chapterNumber, roundNumber)) {
      isActive = true;
      tooltipMessage = 'Round completed';
    } else {
      // Sequential unlocking logic for rounds
      if (requiredRound != null) {
        if (!ModuleFourProgress.isRoundCompleted(
    ...
    int stars =
        ModuleFourProgress.getStarsForChapter(chapterNumber, roundNumber);
    String badge = ModuleFourProgress.getBadgeForStars(stars);
  ...
  Widget build(BuildContext context) {
    ...
    final double button1Top = screenHeight * (isMobile ? 0.25 : 0.42);
    final double button1Left = screenWidth * (isMobile ? 0.22 : 0.001);

    final double button2Top = screenHeight * (isMobile ? 0.57 : 0.77);
    final double button2Left = screenWidth * (isMobile ? 0.9 : 0.45);

    final double button3Top = screenHeight * (isMobile ? 0.18 : 0.25);
    final double button3Left = screenWidth * (isMobile ? 0.98 : 0.53);
    return Scaffold(
      body: Stack(
        children: [
          AnimatedOpacity(
              ...
              children: [
                Positioned.fill(
                  child: FittedBox(
                    fit: BoxFit.cover,
                    child: Image.asset(
                      'assets/images/ModulesBG/Module4BG.png',
                    ),
                  ),
                ),
                Positioned(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        _buildChapterWithTopics(
                                          'Module IV: Repetition Control Structures',
                                          [
                                            "Welcome to Module IV! In this module, you'll explore the world of loops in Java, which are crucial for executing repetitive tasks efficiently. Get ready to dive into the while Loop, the do-while Loop, and the for Loop. Understanding these loop structures will empower you to write more dynamic and powerful programs. Let's get started and master the art of looping in Java!"
                                          ],
                                        ),
                                      ],
                                    ),
