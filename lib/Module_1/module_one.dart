...
import 'package:code_quest_pre/Module_1/Chapter_1/chapter_one.dart';
import 'package:code_quest_pre/Module_1/Chapter_2/chapter_two.dart';
import 'package:code_quest_pre/Module_1/Chapter_3/chapter_three.dart';
import 'package:code_quest_pre/Module_1/module_one_progress.dart';
import 'package:flame_audio/flame_audio.dart';
import 'package:flutter/material.dart';
import 'dart:ui';

class ModuleOneMap extends StatefulWidget {
  ...
  @override
  _ModuleOneMapState createState() => _ModuleOneMapState();
}

class _ModuleOneMapState extends State<ModuleOneMap> {
  final double button1Top = 250;
  final double button1Left = 100;
  final double button2Top = 565;
  final double button2Left = 450;
  final double button3Top = 180;
  final double button3Left = 1050;
  ...
  // Determine if a round in a specific chapter is completed
  ...
  bool _isChapterAvailable(String chapter) {
    if (chapter == 'Chapter I') {
      ...
      return ModuleOneProgress.isChapterCompleted(1) ||
          ModuleOneProgress.isChapterPermanentlyUnlocked(
              1); // Chapter II remains unlocked once Chapter I is done
    } else if (chapter == 'Chapter III') {
      return ModuleOneProgress.isChapterCompleted(2) ||
          ModuleOneProgress.isChapterPermanentlyUnlocked(
  ...
  Widget _buildRoundButton(String text, {int? requiredRound}) {
    ...
    if (ModuleOneProgress.isRoundCompleted(chapterNumber, roundNumber)) {
      isActive = true;
      tooltipMessage = 'Round completed';
    } else {
      // Sequential unlocking logic for rounds
      if (requiredRound != null) {
        if (!ModuleOneProgress.isRoundCompleted(chapterNumber, requiredRound)) {
          isActive = false;
          tooltipMessage =
    ...
    int stars =
        ModuleOneProgress.getStarsForChapter(chapterNumber, roundNumber);
    String badge = ModuleOneProgress.getBadgeForStars(stars);
    ...
    return Tooltip(
      ...
      child: _RoundButton(
        ...
        onPressed: isActive
            ? () {
                // Navigate to the corresponding round
                if (chapterNumber == 1) {
  ...
  Widget build(BuildContext context) {
    ...
    final double button1Top = screenHeight * (isMobile ? 0.25 : 0.35);
    final double button1Left = screenWidth * (isMobile ? 0.15 : 0.001);

    final double button2Top = screenHeight * (isMobile ? 0.5 : 0.7);
    final double button2Left = screenWidth * (isMobile ? 0.4 : 0.3);

    final double button3Top = screenHeight * (isMobile ? 0.15 : 0.2);
    final double button3Left = screenWidth * (isMobile ? 0.9 : 0.55);
              ...
              children: [
                Positioned.fill(
                  child: FittedBox(
                    fit: BoxFit.cover,
                    child: Image.asset(
                      'assets/images/ModulesBG/Module1BG.png',
                    ),
                  ),
                ),
                Positioned(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        _buildChapterWithTopics(
                                          ...
                                          [
                                            "Welcome to Module I! This module will introduce essential Java programming concepts that form the building blocks of coding. Here, you’ll tackle questions on topics like Dissecting Your First Java Program, understanding Java Comments, Statements and Blocks, Identifiers and Keywords, and more. You'll also explore Java Literals, Primitive Data Types, Variables, Operators, and Operator Precedence. Sharpen your understanding of these core ideas to level up your programming skills. Good luck, and let’s dive into Java!"
                                          ],
                                        ),
                                      ],
                                    ),
                ...
                // Chapter I Button - Always Available
                Positioned(
                  top: button1Top,
...
// Chapter II Button - Available if Chapter I is completed
                Positioned(
                  top: button2Top,
...
// Chapter III Button - Available if Chapter II is completed
                Positioned(
                  top: button3Top,
                  left: button3Left + 70,
                  child: _buildPixelButton(
