...
import 'package:code_quest_pre/Module_5/Chapter_1/chapter_one.dart';
import 'package:code_quest_pre/Module_5/Chapter_2/chapter_two.dart';
import 'package:code_quest_pre/Module_5/Chapter_3/chapter_three.dart';
import 'package:code_quest_pre/Module_5/module_five_progress.dart';
import 'package:flame_audio/flame_audio.dart';
import 'package:flutter/material.dart';
import 'dart:ui';

class ModuleFiveMap extends StatefulWidget {
  ...
  @override
  _ModuleFiveMapState createState() => _ModuleFiveMapState();
}

class _ModuleFiveMapState extends State<ModuleFiveMap> {
  final double button1Top = 210;
  final double button1Left = 100;
  final double button2Top = 565;
  final double button2Left = 800;
  final double button3Top = 180;
  final double button3Left = 890;
  ...
  bool _isChapterAvailable(String chapter) {
    if (chapter == 'Chapter I') {
      ...
      return ModuleFiveProgress.isChapterCompleted(1) ||
          ModuleFiveProgress.isChapterPermanentlyUnlocked(
              1); // Chapter II remains unlocked once Chapter I is done
    } else if (chapter == 'Chapter III') {
      return ModuleFiveProgress.isChapterCompleted(2) ||
          ModuleFiveProgress.isChapterPermanentlyUnlocked(
  ...
  Widget _buildRoundButton(String text, {int? requiredRound}) {
    ...
    if (ModuleFiveProgress.isRoundCompleted(chapterNumber, roundNumber)) {
      isActive = true;
      tooltipMessage = 'Round completed';
    } else {
      // Sequential unlocking logic for rounds
      if (requiredRound != null) {
        if (!ModuleFiveProgress.isRoundCompleted(
    ...
    int stars =
        ModuleFiveProgress.getStarsForChapter(chapterNumber, roundNumber);
    String badge = ModuleFiveProgress.getBadgeForStars(stars);
  ...
  Widget build(BuildContext context) {
    ...
    final double button1Top = screenHeight * (isMobile ? 0.21 : 0.35);
    final double button1Left = screenWidth * (isMobile ? 0.11 : 0.001);

    final double button2Top = screenHeight * (isMobile ? 0.57 : 0.75);
    final double button2Left = screenWidth * (isMobile ? 0.8 : 0.43);

    final double button3Top = screenHeight * (isMobile ? 0.18 : 0.28);
    final double button3Left = screenWidth * (isMobile ? 0.89 : 0.55);
    return Scaffold(
      body: Stack(
        children: [
          AnimatedOpacity(
              ...
              children: [
                Positioned.fill(
                  child: FittedBox(
                    fit: BoxFit.cover,
                    child: Image.asset(
                      'assets/images/ModulesBG/Module5BG.png',
                    ),
                  ),
                ),
                Positioned(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        _buildChapterWithTopics(
                                          'Module V: Java Arrays',
                                          [
                                            "Welcome to Module V, the final module! In this module, you’ll uncover the power of arrays in Java, which are essential for managing collections of data. Prepare to explore topics such as Introduction to Arrays, Declaring Arrays, and Accessing Array Elements. You'll also learn about Array Length and dive into the world of Multidimensional Arrays. Mastering these concepts will equip you with the skills needed to handle data efficiently in your programs. Let’s finish strong and solidify your understanding of arrays in Java!"
                                          ],
                                        ),
                                      ],
                                    ),
